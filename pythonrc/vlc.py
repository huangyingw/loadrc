...
def callvlc(targetFile, start=0):
    if targetFile:
        if platform.system() == 'Darwin':
            if start == 0:
                subprocess.check_call(['/Applications/VLC.app/Contents/MacOS/VLC', '--sub-language', 'Chinese', '--sub-autodetect-file', '-f', '--macosx-continue-playback=2', '--rate=2.0', '--no-volume-save', '--volume', '0', targetFile])
            else:
