...
prune_params=();
prune_files=();
find_result="`echo "$2".fw.findresult |sed  -e "s/\//\_/g"`"
...
find -L "$1" "(" "${prune_params[@]}" "${prune_files[@]}" "-o" "-iname" "$find_result" ")" -prune -o "(" "${find_params[@]}" "-o" "-iname" "makefile" ")" -type f -exec fgrep -inH  $grep_params "$2" {} \; > "$find_result"
